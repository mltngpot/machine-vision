...
def estimatespeedverti(Location1, Location2):
    #Euclidean Distance Formula
    d_pixel = math.sqrt(math.pow(Location2[0] - Location1[0], 2) + math.pow(Location2[1] - Location1[1], 2))
    # defining thr pixels per meter
    p2m = 0.2772540756349118        # (1/142)/0.0254
    d_meters = d_pixel*p2m
    time_constant = 1/5.9
    #distance = speed/time
    speed = d_meters / time_constant
    return int(speed*3.6)

def estimatespeedhori(Location1, Location2):
    #Euclidean Distance Formula
    d_pixel = abs(Location2[0] - Location1[0]) + abs(Location2[1] - Location1[1])
    # defining thr pixels per meter
    p2m = 0.2772540756349118        # (1/142)/0.0254
    d_meters = d_pixel*p2m
    time_constant = 1/5.9
...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if object_id[i] == 2:
          ...
          if len(data_deque[id]) >= 2:
           if (y2-y1) > (x2-x1): 
            object_speed = estimatespeedverti(data_deque[id][1], data_deque[id][0])
            # print(data_deque[id][1])
            # print("and previous")
            # print(data_deque[id][0])
            # print("new set")
            label = label + " " + str(object_speed) + "km/hr" + " " + "Vertical"
           else:
            object_speed = estimatespeedhori(data_deque[id][1], data_deque[id][0])
            label = label + " " + str(object_speed) + "km/hr" + " " + "Lateral"
    ...
    #fps = 1/(cTime - pTime)
    #pTime = cTime
    #cv2.putText(img, "FPS:" + str(int(fps)), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
    return img
